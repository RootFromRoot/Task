...
class CommentsAdapter(var items: ArrayList<Data>): RecyclerView.Adapter<LikesViewHolder>()  {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LikesViewHolder {
        return LikesViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_list_comments, parent, false))
    }

    fun setComments(likes: ArrayList<Data>) {
        this.items = likes
